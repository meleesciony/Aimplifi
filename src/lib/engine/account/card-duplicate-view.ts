...
export function cardDuplicatePushNotes(
  ...
  for (const p of resolvePairs(pairs, cards, false)) {
    ...
    const note = (other: string) =>
      p.sameLabel
        ? `${strength} duplicate: a second entry with this same name looks like the same card, so this may be one payment asked for twice. Accounts lists the choices.`
        : `${strength} duplicate: “${other}” looks like the same card, so this may be one payment asked for twice. Accounts lists the choices.`;
...
export function cardDuplicateTraceBasis(
  ...
  return resolvePairs(pairs, cards, false).map((p) =>
    p.sameLabel
      ? `Two rows listed here are both named “${p.nameA}” and look like the same card reaching Aimplifi twice. ${basisOf(p.confidence, p.reasons)} Both are included in this number; nothing has been adjusted. Accounts lists the choices.`
      : `Two rows listed here — “${p.nameA}” and “${p.nameB}” — look like the same card reaching Aimplifi twice. ${basisOf(p.confidence, p.reasons)} Both are included in this number; nothing has been adjusted. Accounts lists the choices.`,
  );
}
...
export function cardDuplicateAnswerNote(
  ...
  return resolvePairs(pairs, cards, false).map(
    (p) =>
      `${sentenceFor(p)} Both are inside the amount and the card count in this answer, so if they are one card, both figures include it twice. ${basisOf(p.confidence, p.reasons)} Nothing has been adjusted — Accounts lists the choices.`,
  );
}
...
export function cardDuplicateUndatedNote(
  ...
  return resolvePairs(pairs, cards, false).map(
    (p) =>
      `${sentenceFor(p)} If they are one card, the count in this answer is one higher than the number of real cards — no amount here is affected, since neither is in any total. ${basisOf(p.confidence, p.reasons)} Nothing has been adjusted — Accounts lists the choices.`,
  );
}
...
export function cardDuplicateRadarNote(
  ...
  return resolvePairs(pairs, cards, false).map((p) => {
    const who = p.sameLabel
      ...
      : `“${p.nameA}” and “${p.nameB}” look like the same card reaching Aimplifi twice.`;
    return `${who} If they are one card, every cycle in this projection counts it twice, so the dip date may be earlier and the amount to move larger than you actually need. ${basisOf(p.confidence, p.reasons)} Nothing has been adjusted — Accounts lists the choices.`;
  });
}
