...
export const GENERIC_CATEGORY_RULES: GenericRule[] = [
  ...
  // GRILLE?S? (L.12c): \bGRILL\b could not match "Grille" — no word boundary before
  // the trailing -e — so e.g. "Goose Pond Bar Grille" earned no suggestion. The
  // widened token subsumes the old BAR ?& ?GRILL compound (kept for the record).
  { pattern: /\b(RESTAURANT|GRILLE?S?|KITCHEN|BISTRO|DINER|EATERY|TAQUERIA|PIZZA|PIZZERIA|SUSHI|RAMEN|STEAKHOUSE|CANTINA|TRATTORIA|OYSTER|SEAFOOD|NOODLE|BAKERY|BAKESHOP|BAKERS|DELI|CATERING|CREAMERY|ICE CREAM|JUICE|SMOOTHIE|BAR ?& ?GRILL|BBQ)\b/i, categoryId: 'dining' },
  // Auto & transport
...
/** Processor / POS prefixes that are never a payee name. Same list cleanDescriptor already strips on ingest. */
const PROCESSOR_PREFIX_RE =
  /^(SQ \*|TST\*\s*|PAYPAL \*|PP\*|PY \*|DD \*|POS \d+ |PADDLE\.NET\*\s*|FS \*|CKE\*|IN \*|SP \* ?)/i;

/** Bank-feed tokens that are never a payee name (live 2026-09-01 register). */
export function stripPayeeNoise(s: string): string {
  let t = s;
  t = t.replace(PROCESSOR_PREFIX_RE, '');
