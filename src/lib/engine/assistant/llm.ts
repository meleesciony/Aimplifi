...
export function buildIntentPrompt(question: string): string {
  return [
    ...
    '- memory_dividend: Perkins/Housel who-notices reflection on the Coach life-energy list — "what\'s a memory dividend?", "who notices what I buy?", "meant to impress". Never a named store or category, never an amount or date. Not "biggest purchase" (that is largest_purchases), not "what should I cut" (that is what_to_cut)',
...
export function intentFromKind(
  ...
  switch (kind) {
    ...
    case 'memory_dividend': {
      ...
      return reflection?.kind === 'memory_dividend' ? reflection : null;
    }
