...
export function buildIntentPrompt(question: string): string {
  return [
    ...
    '- next_dollar: where an extra dollar goes given the reader\'s own rates (debt vs match vs emergency fund vs investing) — "where should my next dollar go?", "should I pay off debt or invest". Never an amount or date. Not "when will I be debt-free" (that is debt_payoff), not "how much do I need to pay my cards" (that is cash_needed), not "how many months of runway" (that is runway)',
...
export function intentFromKind(
  ...
  switch (kind) {
    ...
    case 'next_dollar': {
      ...
      return extra?.kind === 'next_dollar' ? extra : null;
    }
