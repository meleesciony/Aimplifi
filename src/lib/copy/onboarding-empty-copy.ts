/**
 * First-run empty Home. The page used to promise Cash-Needed in 30 seconds
 * with zero navigation, which is only true after a bank link with cards.
 * CSV is a first-class path (#547): a household can paste history with no
 * connection, so the welcome must not sell only the card-pay flow.
 */
