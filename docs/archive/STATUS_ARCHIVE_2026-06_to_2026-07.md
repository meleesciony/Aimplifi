> **HISTORICAL** — archived 2026-08-04 from `docs/STATUS.md` (verbatim moves, not rewrites).
> Point-in-time record; do not update. Current status lives in `docs/STATUS.md`.

# STATUS archive — 2026-06 through 2026-07
...
`cadenceFromGap` recognized four windows (5–9, 12–16, 26–35, 350–380 days), so a ~91-day or
~182-day gap classified as IRREGULAR and `detectRecurring` dropped the series **before** the
projection filter was ever reached. A quarterly water bill was therefore counted **zero times**
— absent from the spending plan's fixed term, from the cash-needed projection, the forecast, the
calendar AND from `/recurring` — and guilt-free spending was overstated by its whole monthly
share. Unlike L.23 (which admitted a cadence the detector already assigned) this is a detection
**CLASS** change: it can only move a user's detected set toward MORE series, and a false positive
does not merely mis-state a figure — it prints a dated outflow on `/calendar` and can raise a
radar "move $X by <date>" instruction for a bill that does not exist.

**Shipped.** (1) Two bands — QUARTERLY 84–98 days, SEMIANNUAL 175–190 — in a `CADENCE_BANDS`
table. (2) A positive-licence **every-gap rule** for those two cadences only: every gap in the
whole history must itself fall inside the band, not merely their median, because with three
sightings the median of two gaps IS their mean and `[30, 150]` would otherwise read as a
quarterly bill. The four older cadences keep the median-only rule deliberately — raising their
bar would re-detect every existing user. (3) Both are projected as **expenses only and only
while still charging**, which generalized L.23's ANNUAL rule into one shared `LONG_CADENCES`
list so the rule cannot be applied to one cadence and forgotten on another. (4) `monthsPerCadence`
replaced the same four-branch ternary that had been copy-pasted into four expanders
(cash-needed/assemble, forecast, calendar/build, plan's `scheduledOccurrencesBetween`) — a missed
branch there is silent by construction, since the row falls through to the one-occurrence `else`.
(5) Monthly rates /3 and /6, matching `/recurring`'s own `PER_MONTH` (two tables held together by
a test rather than merged, because they disagree about IRREGULAR on purpose and the plan's exact
integer form must not move by a rounding ulp).

**Demo impact: none, verified by probe rather than assumed.** Of 47 merchant groups in the seed,
four have a median gap inside the new bands — Costco Gas 89, Zelle Payment 91, Etsy 86, Kroger 97
— and all four are killed by the EXISTING amount-stability filter (5–7 distinct amounts against a
limit of 2). Those four names are also exactly the shape the false-positive risk takes: sparse
variable real-world spending that happens to average a quarter apart. The seed still detects one
BIWEEKLY and eleven MONTHLY series, and the probe is kept as a test.

**Mutation testing (4 mutations, all killed).** Neutering `monthsPerCadence`'s two new branches
killed exactly the three expander locks; emptying `EVERY_GAP_CADENCES` killed the two licence
tests; shrinking `LONG_CADENCES` back to ANNUAL-only killed the projection and lapse locks;
widening the spread cap to 999 killed the two rhythm-agreement locks.

**Two fresh-context hostile critics, both FAIL, every finding executed and every P1 fixed.**

The **money-math critic** (1 P1, 3 P2, 4 P3) broke the every-gap licence with the smallest
possible counterexample: the quarterly band is 15 days wide, so three haircuts **84 and 98 days
apart** put BOTH gaps inside it, the licence passed them, and a discretionary purchase became a
projected bill with a date on the calendar and a $15.00/month charge against guilt-free spending
— including via the two-plateau price-change path (three vet visits at $100/$100/$250). Fixed by
requiring the gaps to agree with EACH OTHER as well as with the band: `max - min <= 7` days,
which costs no genuine bill (real anchors cluster within 3). It also found that
`monthlyRateCents` and `/recurring`'s `PER_MONTH` **disagreed by a cent on BIWEEKLY** at 120,989
amounts under $20k — `Math.round(a*26/12)` against `Math.round(a*(26/12))`, since 26/12 is
inexact in binary — including a $2,307.69 paycheck, i.e. an ordinary $60k salary; worse, the new
"two tables, one fact" lock passed anyway because every amount it probed divided its factor
exactly. `PER_MONTH` is now an exact numerator/denominator pair and the lock fuzzes residues. Two
bounds it found are recorded rather than fixed (EDGE_CASES §C3/§C4): a bill at a band EDGE is
under-counted by up to ~8.7% because the rate is a flat 1/3, in the dangerous direction and
pre-existing for ANNUAL; and month-end anchors drift a day per clamp. It could NOT break
`monthsPerCadence` equivalence (7,938-case sweep, 0 differences), the lapse gate (80-case sweep,
0 differences), double counting anywhere, the new cadences' rate tables (2,000,000 amounts), any
real-world anchor, or golden safety (12 seed series at three different `asOf` dates).

The **copy/claims critic** returned **FAIL — 4 P1, 4 P2, 6 P3**, every finding executed, and
every P1 plus every actionable P2/P3 fixed: the disclosure
list had become a *closed world* that was false (seven IRREGULAR ranges survive, and the copy
named four of them — a four-monthly utility bill, a real US billing period, was covered by
nothing); the new precondition sentence promised a recovery path that does not exist ("we don't
count it **yet**" — the every-gap rule reads ALL of history with no lookback window, so a
13-sighting quarterly bill with one 14-day-late cycle two years back is $0 **permanently** and no
future on-time cycle brings it back); the quarterly/semiannual INCOME asymmetry L.23 had
disclosed for ANNUAL was reintroduced undisclosed at all three sites; and the smoothing sentence
lifted ANNUAL's "in **the month** the bill leaves your account", which is right once a year and
wrong four times a year — a reader budgeting one lump for a $300 quarterly bill under-plans by
3 × $200. The critic also caught a **false comment** claiming an e2e bound a testid that nothing
binds (now bound), an early cycle being undisclosed while only lateness was named, and this file
itself contradicting the code in three places (fixed above). It could NOT break the fractions,
the ANNUAL byte-identity, the history figures, the gating logic, or any expander.

Also corrected during the build, by a test rather than by review: three source comments claimed a
quarterly row "genuinely CAN recur inside a 90-day horizon". It cannot — a quarterly period is
91–92 days, longer than the widest horizon in the app — so in forecast and cash-needed it appears
at most once, exactly like an annual bill. The reachable differences are the calendar's
multi-month windows (three clicks) and a stale anchor self-healing forward.

### 🟠 OPEN after L.24

1. **Every rhythm between and beyond the six bands still counts ZERO** (dangerous direction, and
   the narrowed remainder of L.23 OPEN #1). Executed, the surviving IRREGULAR ranges are 1–4,
   10–11, 17–25, 36–83, **99–174**, **191–349** and **381+** days — so ten-day, three-weekly,
   six-weekly, bi-monthly (~61), **four-monthly (~122, a real US utility period)**, five-monthly,
   eight-to-eleven-monthly and every-18-months bills are all absent from the plan, the projections
   and `/recurring`. Each further band is its own detection-class change. Now disclosed on
   /spending-plan rather than implied away.
2. **The every-gap licence is permanent and unforgiving** — one cycle more than ~a week off in
   either direction keeps a quarterly/semiannual series out **forever**, because detection reads
   all of history and later on-time cycles cannot retire the bad gap. Deliberate (a missed
   detection leaves the status quo; a false positive invents a dated obligation), disclosed in
   those words, but a windowed or tolerant variant is the obvious follow-up.
3. **A quarterly bill goes uncounted after ~137 days of silence** (the lapse gate, `91 × 1.5`).
...
6. **A bill at a band EDGE is under-counted by up to ~8.7%** (money critic P2-3): the band admits
   84–98 days but the rate is a flat 1/3, so a bill every 84 days charges 4.35×/yr and is counted
   4×. Direction: raises guilt-free spending. Pre-existing for ANNUAL (350-day rhythm, same
   shape); recorded in EDGE_CASES §C3. Fixing it means rating by observed median gap, a new
   rating model for every cadence.
7. **A latent seed-vs-detected double count** (money critic P3-5, pre-existing and currently
   UNREACHABLE): `refreshRecurringForUser` replaces only `source in ('payroll-detected',
   'recurring')` rows while `getSpendingPlan` sums every negative scheduled row, so a user with
   BOTH seed rows and detected rows would count rent twice ($1,800/mo phantom on the demo). Every
   caller is demo-fenced and a real user has no `seed` rows, but L.24 adds two more cadences that
   ride the same path — worth a guard.
8. **`today-feed-copy.ts` still has an unconverted cadence ternary** that would print "monthly"
   for a long cadence (money critic P3-7). Verified unreachable — `isPauseCadence` gates the
   pause detector to W/B/M — but it is the one chain L.24 did not convert, and it is precisely
   the silent-fallthrough class `monthsPerCadence` exists to prevent.
9. **In the month a long bill actually lands, the plan has reserved only its share** — unchanged
   from L.23 OPEN #4 and now true four times a year rather than once, which is why the copy had
   to stop saying "the month".
...
1. ~~**A QUARTERLY or SEMIANNUAL bill is still counted ZERO times**~~ — **CLOSED in L.24**
   (§L.24 above). The two bands are recognized, projected as expenses only and only while still
   charging. What remains of this item is narrower and restated as L.24 OPEN #1: every rhythm
   between and beyond the six recognized bands still counts zero.
...
6. **The lapse gate applies to the LONG cadences only** (was ANNUAL-only; L.24 extended it to
   ...
   widening it to W/B/M would change what is projected for every existing user, in the direction
   of dropping bills. Pinned by a test, so the asymmetry is visible rather than assumed.
...
1. ~~**A DETECTED annual/quarterly bill is counted ZERO times in the fixed term**~~ — **CLOSED
   in full**: the ANNUAL half in L.23, the QUARTERLY/SEMIANNUAL half in L.24 (both below). (The original entry also said the `/12` branch was "live only for
...
## 🟠→✅ FIXED — intermittent DOM duplication on /accounts (surfaced as a strict-mode e2e failure)
...
at the bottom of this section. FIXED the same day — read "The fix, and the two leads it killed"
immediately below. The sections between are the 2026-07-24 narrowing, kept because every
elimination in them still holds.**

### The fix, and the two leads it killed (O.3, 2026-07-27)

**Both halves of the recorded next step were wrong, and each cost one command to kill.**

1. *"A push landing while `$RV` drains rides a queue about to be wiped."* **No such race exists.**
   `$RV` is synchronous — it walks the queue and ends with `a.length = 0` — and no script can run
   between its iterations, because DOM mutation dispatches nothing synchronously. The real
   fragility was in the same expression all along and is a bootstrap condition, not a race:
   `$RC` chooses its scheduler with `"number" !== typeof $RT ? requestAnimationFrame(…) :
   setTimeout(…)`, and `$RT` is assigned on the **first line of `$RV`**. So exactly one reveal per
   document — the first — depends on a frame; every later one runs on a timer that does not. A
   renderer that never paints strands the first boundary permanently, and only the first.
2. *"If that is it, the fix is a react-dom upgrade."* **There is no version to upgrade to.**
   React's experimental channel (`next/dist/compiled/react-dom-experimental/cjs/
   react-dom-server.edge.production.js`) ships this instruction set **byte-identical** to stable.
   Note also that the HTML is generated by Next's *compiled* React, so bumping the `react-dom`
   entry in `package.json` would not have changed a byte of it either way.

**Reproduced deterministically, by cause rather than by load.** Every attempt to trigger this with
contention failed (96 instrumented loads, zero duplications). Constructing the condition it needs
took one page load: replace `requestAnimationFrame` with a no-op and a plain `/accounts` leaves

```
STARVED   { stagingContainers: 1, rbLength: 2, rtType: "undefined",
            staging: [{ id: "S:0", childElements: 18, testIdsInside: [accounts-empty, …] }] }
CONTROL   { stagingContainers: 0, rbLength: 0, rtType: "number" }
```

`$RT`'s *type* is the whole measurement: it is written by the code under suspicion, so `undefined`
**proves** `$RV` never executed rather than suggesting it. The card is not missing and not
duplicated by the app — it is parked, whole, in a hidden staging div React never got a frame to
empty.

**The fix: `tests/e2e/helpers/test.ts`.** A 32ms interval, installed on every page, that calls
`$RV($RB)` when the queue is non-empty — doing exactly what React's own rAF callback would have
done, on a clock instead of a frame. It is idempotent (`$RV` empties the queue, so a drain racing a
real frame finds nothing to do), and after the first drain `$RT` is a number, so React's own
scheduling switches to `setTimeout` and the page needs no frames again. Under the same starvation
the readings become **byte-identical to CONTROL**.

**This is a harness fix, and that is the honest description.** A real browser paints, so a real
reader always gets the frame; the orphan sits outside `#content`, unreachable by any reader, screen
reader or `getElementById`. What makes the drain legitimate rather than a locator loosening is that
it cannot hide the defect these specs exist to catch: **a genuine duplicate render puts both copies
inside `#content`, where every strict locator still resolves to two.** No locator was touched, no
spec retitled, no assertion weakened.

**The fence holds by construction, not by memory** — it had to reach 56 spec files, and a fence
copied per call site misses call sites. The drain lives in one module; it is patched onto
`browser.newContext` so the three specs that build their own context (notifications,
payment-reminders, phase4-features) inherit it; and an eslint `no-restricted-imports` rule on
`tests/e2e/**/*.spec.ts` makes importing `test`/`expect` from `@playwright/test` an error, so a
future spec cannot opt out silently.

**Standing lock: `tests/e2e/streamed-reveal.spec.ts`** — starves rAF, then requires the content
visible, `$RT` a number, `$RB` empty and no staging container left. Two guards stop it passing
vacuously: a rAF scheduled from the test must NOT call back (the hard case is genuinely present),
and `$RT` must be a number (a boundary really was streamed *and* drained — on a page that never
suspends it stays `undefined` and the test fails). Mutation-proven: disabling the drain fails it
with `Received: hidden` after 43 resolutions of a present-but-hidden element — the reported
signature exactly.

**Measured where the flake actually lives — full-suite runs at the default 4 workers, never a
targeted run:**

| tree | full-suite runs | result |
|---|---|---|
| before (recorded 2026-07-27) | stashed CLEAN tree | **5 failed** |
| before (recorded 2026-07-27) | O.1 applied, three runs | **2, then 1, then 6 failed** |
| **after (this session)** | three consecutive runs | **209/209, 209/209, 209/209 — 0 failed** |

The suite is now **209 tests in 56 files** (`npx playwright test --list`) — the 208 that O.1's gate
ran, plus the streamed-reveal lock. `verify.sh` discriminates again: a real regression in the four /accounts reconciliation specs is
visible instead of being written off as noise.
...
has a same-month return; netting would risk a confusing negative line)
— **SUPERSEDED by O.8a 2026-07-29:** refunds ARE netted now, and the confusing
negative is answered by dropping a merchant whose net is ≤ 0, the rule
`spendingByCategory` already applies to a net-refunded category; (4) the
...
**AMENDED by O.8a 2026-07-29:** the line runs THROUGH the new-merchant card rather than around it. Which
merchants are new is still settled-only (a naming claim about an event); the AMOUNT beside each one is an
aggregate at merchant scope — the identical question Ask's `merchantSpend` answers — so it counts pending
and nets refunds. Biggest purchases is unchanged: it names a single row.
...
## 2026-07-29 — O.8: one basis for a merchant aggregate, and a proposed money fix refuted (verify green)

**The gap, measured before anything changed.** /trends' "New this month" card and Ask answered the same
question — "how much did I spend at merchant M this month" — with different dollars. Four rows at one
brand-new merchant (two settled purchases, one pending purchase, one settled refund) produced **$65.00**
on /trends and **$80.00** in Ask. Both sentences were true of their own basis and both were disclosed on
their own surface, which is exactly why nobody could see it: the two are never shown side by side.

**The decision: the split runs THROUGH the card, not around it.** WHICH merchants are new is a claim
about an EVENT, so it stays settled-purchase-only — a pending authorisation can vanish, and a merchant
named on one that never posts is a sentence about something that did not happen. The AMOUNT beside each
is an AGGREGATE at merchant scope, so it now reads the reports engine's own exported `isSpendRow` /
`spendContributionCents`: posted and pending, refunds netted, bucketed by the stored category.
`computeSpendingTrends` passes every row and each insight applies its own narrowing, so a basis lives
beside the claim it describes. #74's accepted gross simplification ("netting would risk a confusing
negative new-merchant line") is answered by dropping a net-≤-0 merchant — the rule `spendingByCategory`
already applies to a net-refunded category, not a newly invented one.

**O.8(b) refuted, and then the refutation's own argument falsified.** TASKS O.8 asked for the
`credit-card-payment` category to be excluded from spending, on two grounds that are both wrong.
"/budgets already excludes it via NON_BUDGETABLE" — false; that set gates which categories may carry a
budget TARGET, while `summarizeBudgets` renders the union of spend keys, so the two surfaces never
disagreed. And the double count it feared is already handled: a read-only production probe found
**20 of 20** `credit-card-payment` rows carrying `isTransfer: true` (the defect set is empty), because
the pair detector catches them whenever we hold the card. For contrast, the id-based `transfer`
exclusion is doing real work on the same data — **49 rows**, signed −$14,872.95, that the flag alone
never caught. *(Counts live here rather than in source comments, per the one-status-home rule.)*

But the first version of that argument said excluding "would fix nothing", and a critic executed the
counterexample: pairing needs opposite amounts within **±3 calendar days**, so a payment leaving checking
on the 28th whose card-side credit posts on the 3rd escapes — only the card side is flagged, the payment
counts as spending on top of the charges it settles, and the credit never repays it because next month's
net-refund rule drops it. The exclusion is still declined, but as a **trade-off chosen by failure
direction**: it would also delete the only trace of money leaving for a reader paying a card this app
cannot see, and an understated figure beneath "$87.70 left this month" is the failure that costs money
(L.14). The straddle is locked as a known defect and carried to **O.10b**, where the fix belongs — in the
detector, which repairs the double count without deleting anything.

**What the two critics found (both FAIL, converging INDEPENDENTLY on the same P1).** I claimed the new
amount reads "the exact rows Ask counts" and that the two "cannot drift". False, and false on the shipped
demo seed by 5.2×: `merchantMatches` takes a bidirectional whole-word PREFIX, so "Costco Gas" sweeps in
every "Costco" row — **$37.38** here against **$195.82** there, answered under the name *Costco*.
Sharing a row BASIS is not sharing a merchant SCOPE. The gap predates this slice (the old settled-gross
figure was the same $37.38), so what shipped was a false claim rather than a regression; every claim is
narrowed and the divergence is now PINNED by assertions, so closing it (O.10a) means changing a test that
explains itself. The parity lock itself was the second P1: every case used one merchant string, which
makes exact-key and prefix matching trivially identical — a fixture that cannot express the failure it
advertises catching. The seed assertion had the same disease inverted (`amountCents > 0` became
tautological the moment the net-≤-0 drop landed); replaced with golden literals. The claims critic then
falsified my own basis line — "a merchant appears here once a purchase settles" is broken by the drop
rule, and a PENDING refund can veto a merchant a settled purchase confirmed, so pending money cannot name
an event but can un-name one. Rewritten and both cases locked.

**Also fixed from the critics:** production counts hardcoded into source comments, a probe printing user
emails into transcripts, the live `.env.prod.tmp` credential left at repo root (deleted; regenerate with
`vercel env pull` when a probe needs it), and the new-merchants card having **no test of any kind** — its
basis sentence could have been deleted with every suite green. The e2e written for it failed on first run
against a stale build (the recorded tell: "my change had no effect at all") and passed after a rebuild.

**Open, carried to Wave O.10, each verified by execution:** Ask's prefix-family merchant scope (O.10a),
