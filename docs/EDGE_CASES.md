...
**Deferred to slice 5 (display surfaces, same-deploy rule per critic F5):** the `/accounts` page +
`getAccountsView` net-worth trend + the assistant's account-balance answer, which read Prisma directly
rather than the snapshot — see spec §10.
