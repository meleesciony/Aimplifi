...
- **Habits & Plan Adherence — streaks + did your automation run? (#17, M, ✅ STREAKS HALF SHIPPED 2026-07-21, #254; the drift loop stays gated).** The groundable streaks are now BUILT per the split below: savings-rate streak/personal-best had shipped at #205; #254 adds the card cleared-in-full streak (pure `engine/cards/cleared-streak.ts` — resolved = due-date strictly past, cleared = payments dated ≤ dueDate cover the balance, month qualifies when every resolved statement cleared; seed shows 17 months across 4 cards) and the no-subscription-creep streak (pure `engine/recurring/creep-streak.ts` — creep = a detected price INCREASE on an `isSubscription` series at ym(priceChangedAt), full-month walk capped at a disclosed 12; seed shows 3 months since the Netflix increase, facts inline). One /coach Habit Streaks card; basis stated inline; no persistence, no LLM. See STATUS §Habit Streaks + DECISIONS #254. *The drift-loop half ("blueprint said $500, only $300 landed") stays gated on its original grounds:* `detectTransfers` returns only a `Set<string>` of ids with no destination/pairing/confidence, and the app models savings as an income−expenses residual, not a tracked flow. Rework unchanged: build a real transfer-pair engine (destination + confidence + a correction trail), hard-gate on both-legs-connected, descope to aggregate intended-vs-inferred.
