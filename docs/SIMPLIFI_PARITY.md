...
| 6b | Category list **edit / delete**, and a category set that matches a real user's | **SHIPPED 2026-07-31 (O.17, DECISIONS #350).** The built-in list grew 91 -> 113 from the owner's own export (redundant and user-specific rows refused, each named in #350). Every built-in can now be **renamed** per user and **removed** from the pickers, from Settings -> Categories; custom categories already had create/rename/delete. A built-in cannot be truly deleted and we say so rather than pretending: those rows are global and are the FK target of every user's history, so "Remove" is the honest word for hiding it, and the copy states that anything already filed keeps its category, still counts in reports, and can still be auto-filed there. The rename is an overlay (`CategoryRename`) resolved through the per-user meta, so it reaches the pickers, the register, the detail view, reports, trends, budgets, coach, recurring, Ask, /rules and the CSV export. Still absent: Simplifi's **Expense/Income type** toggle, which row 6 defers with cause. | **O.17** |
| 7 | **Tags** (many per row, filterable) | **MISSING.** No column, no join table. | O.11d |
| 8 | **Exclude from** budgets / reports, per row | **SHIPPED 2026-07-30 (O.15 slice 2)** as `Transaction.excludeFromTotals`, set from the row action menu and the detail view, badged in the register so an excluded row never looks like a counted one, and read through ONE shared predicate (`src/lib/engine/transactions/exclude.ts`). *As authored this row read MISSING, and it still read MISSING on 2026-07-31 while row 4 of this same table already recorded the ship — corrected during O.13h.* Still absent as a rule THEN-action (row 2). | O.15 slice 2 |
...
| 14 | **Attach a receipt / file** | **SHIPPED 2026-07-31 (O.13h).** A receipt or document attaches to any transaction from the detail view: upload, preview, download, remove. Six types (JPEG/PNG/GIF/WebP/HEIC/PDF), 5 MB each, 5 per transaction. The stored content type is SNIFFED from the bytes, never taken from the filename or the browser's declaration, because the stored type is what the download route echoes back. Bytes live in this database rather than object storage so the retention policy's single-cascade deletion promise holds by construction (DECISIONS #349). Visible ONLY to the account's owner — a household partner with shared access to the account never sees them. | **O.13h** |
