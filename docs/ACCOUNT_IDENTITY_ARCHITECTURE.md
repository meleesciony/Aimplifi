...
| D10 | **A balance that has stopped updating may not be presented as one that is updating** (TASKS L.14, 2026-07-25). An account a connection stops returning — the ordinary result of unticking one in Link update mode — keeps its row, its balance, and its place in every total, deliberately: only the user knows whether the account still exists, and silently removing a still-owed card from cash-needed risks a missed payment, which is the more expensive direction. What may NOT survive is the claim. Its freshness stops being graded from its bank's sync date (`not_shared`), the surfaces that carry a total say the balance is frozen and still counted, and Delete is permitted, because the standing refusal — "the next sync would bring this row back" — is false for a row the feed no longer sends. Absence counts as evidence ONLY from a complete, wholly-readable `/accounts/get` census scoped to rows provably belonging to that connection: never from the partial `/transactions/sync` echo, never from an empty or malformed list, and never re-stamped, since the first observation is the one the disclosure claims. Residual: a row whose `plaidItemId` predates #256 is out of scope (it cannot be proven to belong to the connection), and SimpleFIN is unwired — both fail toward the pre-existing behaviour, never toward a false drop. |
