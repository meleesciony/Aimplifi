...
| T9 | Joint answer never counts an account twice (self-share); both-partners-connect-the-same-real-bank is DETECTED AND DISCLOSED, never silently mailed (the disjoint-by-id proof cannot see two provider rows for one real account — slice-8 critic F-5); #192 detector input stays the owned set for a household viewer | merge unit: disjoint-by-account-id proof (clause a) + `detectHouseholdDuplicateAccounts` units + dashboard/digest disclosure integrations (clause b, advisory — figures deliberately NOT adjusted) + detector-input-set unit (clause c) |
