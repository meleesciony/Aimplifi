...
- [mobile-380 Playwright viewport-scaling flake](mobile-380-viewport-scaling-flake.md) — this machine renders the 380×800 mobile project at ~425×895 (an ~11.8% Chromium/Windows scaling mismatch), so fixed-bottom-nav clicks intermittently-but-reproducibly intercept; confirmed pre-existing via `git stash` A/B control, not caused by any one diff — don't chase it inside an unrelated task.
