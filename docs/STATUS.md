...
**Still open / residuals.** (1) **Critic P2-4:** the card's "Suggested: about $X/mo" is solved over the same frozen balances and the note is scoped to the total only — a wider claim would be false once `updateGoalMonthly` replaces the monthly, and the row carries no "monthly hand-edited" fact; next cut if taken: a second stamp or a claim that names the monthly only while `monthlyContributionCents` still equals the save's value. (2) **Critic P2-6:** "saved on <date>" is `businessToday`'s server-local calendar date (inherited from #58, shared by every "today"; Vercel `TZ` UNVERIFIED). (3) `text-xs` muted contrast unmeasured (pre-existing token). Wave 0 ops owner-blocked. M.4 owner-deferred.
