...
**Live:** `scripts/o18c-live-deploy-check.mjs` **12/12 against production** on the ebb987c deploy — the /recurring rows carry the charges toggle ("Show 18 charges" on Netflix), the panel lists the 18 charges newest-first with signed amounts, the basis embeds the row's own $17.99 with "typical amount, not the total of 18 charges", the plateau sentence names both amounts, payroll reads "Show 39 deposits" + "most recent deposit" + biweekly, collapse + aria follow, zero page errors. (The fix commit below re-ships the F1/F4 sentences; the check re-runs against it.)
