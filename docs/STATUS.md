...
**A read-only production probe was written first, and it falsified that.** Replaying the engine per real user (`scripts/audit-probes/o20g-creep-income-refunds.mts`, the C.19 pattern) showed the gating ALONE made the owner's own figure **worse**: 67 positives totalling $38,619.68 left his income series, the first-half income median fell from **$10,604.95 to $0.08** (one interest credit — the month before it carried 59 rows and zero income rows), and income growth went from 592% to **70,470,525%**. That number never printed, because the sentence carrying it only renders when flagged — but it silenced the flag, so his /coach page said "Spending growth is tracking income growth — no lifestyle drift detected" while his discretionary spending grew ~153%. Shipping the row as written would have left that false all-clear in place and made the underlying figure seven orders of magnitude worse.
