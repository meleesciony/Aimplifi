...
**CI (run 31450796439 on 23f5485): SUCCESS — gate GREEN, first attempt** (no rerun; the whole `VERIFY_E2E=1` suite passed in the run the push triggered). **Live:** `scripts/o18efu2-live-deploy-check.mjs` result recorded here when run against production (marker: the `figureless` scope literal + the two render testids in the /reports and /trends client bundles — the demo corpus has no exclusions, so the sentence never renders on demo and bytes are the only honest discriminator).
