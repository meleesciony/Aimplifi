...
**Ship.** `e36eee79` pushed to `origin/main` (no `prisma/` diff — database untouched). CI verify run 34737014759 — verdict recorded below when read. Vercel deployment on the sha: **success** (commit status). Live probe (demo session on `www.aimplifi.app`): /coach creep panel renders **"the same Fixed or Discretionary label the register shows"**, the old divergence sentence is gone, December lists 15 rows, verdict "Spending is outpacing income" — the deployed build carries the slice.
