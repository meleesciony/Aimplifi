...
function decisionSources(): string {
  const archives = existsSync(ARCHIVE_DIR)
    ? readdirSync(ARCHIVE_DIR)
        ...
        .sort()
    : [];
  return [DECISIONS_PATH, ...archives.map((f) => join(ARCHIVE_DIR, f))]
    .map(readFile)
    .join("\n");
}
...
function regenerateDecisionsIndex(): void {
  const contents = decisionSources();
  ...
  const header = [
    "# Decisions Index",
    "",
    "Auto-generated by `scripts/ledger.ts` — one line per decision in `docs/DECISIONS.md`",
    "and its verbatim rotation archives (`docs/archive/DECISIONS_ARCHIVE_*.md`), both",
    "formats (the legacy `| n | … |` table rows and the `## #n — title` sections), so a",
    "session can find a decision by number or keyword without loading the full ledgers.",
