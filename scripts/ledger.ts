...
/**
 * Since the 2026-08-27 ledger rotation, decision text lives in docs/DECISIONS.md
 * PLUS its verbatim archives (docs/archive/DECISIONS_ARCHIVE_*.md — the lower
 * numbers; the live file's pointer banner says the numbering never resets).
 * Anything that enumerates decisions must read the UNION — an index/next-number
 * pass over the live file alone is the same silent deletion the refusal guard
 * below exists to prevent.
 */
function decisionSourceList(): { file: string; contents: string }[] {
  const archives = existsSync(ARCHIVE_DIR)
    ...
    : [];
  return [
    { file: "docs/DECISIONS.md", contents: readFile(DECISIONS_PATH) },
    ...archives.map((f) => ({
      file: `docs/archive/${f}`,
      contents: readFile(join(ARCHIVE_DIR, f)),
    })),
  ];
}

function decisionSources(): string {
  return decisionSourceList()
    .map((s) => s.contents)
    .join("\n");
}
...
function appendDecision(phase: string, decision: string, rationale: string): void {
  ...
  const num = nextDecisionNumber(decisionSources());
...
function regenerateDecisionsIndex(): void {
  const eol = detectEol(readFile(DECISIONS_PATH));
  const entries = collectFromSources(decisionSourceList());
  ...
  if (dupes.length > 0) {
    throw new Error(
      `ledger.ts reindex: the decision sources define these numbers more than once: ` +
    ...
    if (dropped.length > 0) {
      throw new Error(
        `ledger.ts reindex: REFUSING to write — ${dropped.length} decision(s) indexed today ` +
          ...
          `docs/DECISIONS.md and its archives, or they use a format this parser does not recognise ` +
