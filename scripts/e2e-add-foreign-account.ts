 ...
 *
 * Usage: npx tsx scripts/e2e-add-foreign-account.ts <user-email@aimplifi.test>
 */
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const refs = ['e2e-fx-usd', 'e2e-fx-eur', 'e2e-fx-gbp'];
    ...
    await prisma.account.createMany({
      data: [
        ...
        { userId: user.id, provider: 'simplefin', providerRef: 'e2e-fx-usd', name: 'E2E US Checking', type: 'CHECKING', currentBalanceCents: 1_000_00, currency: 'USD' },
        // Two withheld foreign accounts → banner: "2 accounts not included", "EUR, GBP".
    ...
    console.log(`[e2e-add-foreign-account] added 1 USD + 2 foreign accounts for ${email}`);
  } finally {
