/**
 * Phase-2 baseline walkthrough (PULSE_CATEGORIZATION_FIX): drive the CURRENT triage
 * flow over the messy 60-day dataset exactly as a user would, on a 380x800 mobile
 * viewport, counting every real interaction and timing every action. Produces:
 *   docs/baseline/phase2/shots/*.png       — screenshots of each distinct UI state
 *   docs/baseline/phase2/baseline-run.json — the full interaction log + metrics
 *
 * User policy (naive-efficient — favorable to the current design):
 *   • suggestion matches the human label → batch when offered, else Accept (1 tap)
 *   • wrong suggestion → Pick (1 tap) → alternative if it's one of the 3 (1 tap),
 *     else search (1 interaction) + tap the category row (1 tap)
 *   • rule prompts are ignored (0 taps — they are non-blocking)
 * Interactions counted = taps + search-fills. Human time is reported via the
 * repo's documented budget (4.0s/interaction, phase2-triage.spec.ts:9-11) PLUS the
 * raw bot wall-clock — labeled separately; neither is presented as the other.
 *
 * Stall guard: any single action taking >45s is logged as a STALL (machine issue,
 * STATUS #16/#17); after 2 stalls the run aborts with an honest partial report.
 *
 * Usage: npx tsx scripts/baseline-triage-walkthrough.ts [--keep-server]
 */
...
const OUT_DIR = path.join(process.cwd(), 'docs', 'baseline', 'phase2');
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const persist = (aborted: string | null) => {
      ...
      fs.writeFileSync(
        path.join(OUT_DIR, 'baseline-run.json'),
    ...
    while ((await remaining()) > 0) {
      ...
      const raw = (await card.locator('p.font-mono').textContent())?.trim() ?? '';
      const cardDate = (await card.locator('p.text-xs.text-muted-foreground').nth(1).textContent())?.trim() ?? '';
      const suggestion = (await page.getByTestId('triage-suggestion').textContent())?.trim() ?? '';
      ...
      if (suggestion === intended.name) {
        const batch = page.getByTestId('triage-batch');
        const useBatch = await batch.isVisible().catch(() => false);
        const a0 = Date.now();
        try {
          if (useBatch) {
            await shotOnce('05-batch-offer.png');
            await batch.click();
          } else {
            await page.getByTestId('triage-accept').click();
          }
          await page.waitForFunction(
            (n) => Number(document.querySelector('[data-testid="triage-inbox"]')?.getAttribute('data-remaining')) < n,
            before, { timeout: ACTION_TIMEOUT },
          );
          record(useBatch ? 'batch' : 'accept', Date.now() - a0, await remaining());
          await shotOnce('06-rule-prompt.png'); // often visible right after an accept
        } catch { await stalled(useBatch ? 'batch' : 'accept'); }
      } else {
    ...
    fs.writeFileSync(path.join(OUT_DIR, 'baseline-run.json'), JSON.stringify(result, null, 2));
