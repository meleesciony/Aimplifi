/**
 * Spending Plan (DECISIONS #66; #295 guilt-free reframe): demo sign-in → Plan →
 * "guilt-free to spend" headline renders with its breakdown. The exact cents come
 * from the seed, so we assert a money-shaped value + the breakdown structure
 * rather than a brittle pinned number.
 */
...
test('Spending Plan: guilt-free headline + breakdown render for the demo user', async ({ page }) => {
  ...
  // Allocation legend (#186): five labeled swatches under the bar (touch-visible;
