/**
 * Conscious Spending lens (P0.4, DECISIONS #93). The cardinal property: the
 * buckets are a strict re-partition of computeSpendingPlan's quantities, so
 * they must ALWAYS sum back to expectedIncomeCents — there is no second spend
 * definition. Anchored on the real engine output, never on hand-built plans.
 */
...
describe('mapToConsciousBuckets — provably equal re-partition', () => {
  ...
  it('maps the plan quantities to the right buckets with correct shares', () => {
    const b = mapToConsciousBuckets(plan()); // left = 500k - (250k+50k+50k) = 150k
    const byKey = Object.fromEntries(b.buckets.map((x) => [x.key, x]));
    expect(byKey.fixed.cents).toBe(300_000); // spent 250k + upcoming bills 50k
    expect(byKey.savings.cents).toBe(50_000); // planned savings
  ...
  it('stays a strict partition when overspent (guilt-free goes negative)', () => {
    const b = mapToConsciousBuckets(
      plan({ expectedIncomeCents: 300_000, spentSoFarCents: 280_000, upcomingBillsCents: 30_000, plannedSavingsCents: 20_000 }),
  ...
  it('never divides by zero when there is no income', () => {
    const b = mapToConsciousBuckets(
      plan({ expectedIncomeCents: 0, spentSoFarCents: 100_000, upcomingBillsCents: 0, plannedSavingsCents: 0 }),
    );
