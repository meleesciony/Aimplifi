...
describe('linkedWithOverlapFlash — both connections kept, and the overlap said out loud', () => {
  ...
  describe('linkedForHistoryFlash', () => {
    ...
    it('test_regression__names_the_control_because_the_cards_default_points_the_other_way', () => {
      ...
      // combine revokes the dropped Item. H.6c re-ranked `keepRank` on stored FEED depth, so
      // once the background pull lands the prominent button points the right way — but before
      // it lands, the depth rule itself prefers the old side (the new side's stored floor is
      // recent or nothing). This copy and the card's own depth note are the reader's protection
      // in that window, so it stays specific enough to override a wrong default: "the new one"
      // is not enough; it has to name which button that IS.
      const msg = linkedForHistoryFlash(one);
