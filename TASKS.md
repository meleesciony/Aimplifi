...
| 0.5 | Operator "activation checklist" panel in /settings: reads env-var *presence* (never values) and lists which dormant systems (push/email/digest/Sentry/crons) are live. Pure presence-map engine + tests. | Sonnet | medium | 40k | [ ] ready in-repo (no creds needed) — recon done; three `…Configured()` helpers already exist (email/errors/push) to aggregate. |
