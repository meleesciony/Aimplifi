...
|---|------|-------------|--------|------------|--------|
| 0.1 | Push #171–#182 to GitHub; confirm CI (#181) runs verify.sh green. Single-machine loss risk until done. | Human + Sonnet (assist) | low | 10k | [~] pushed 2026-07-09 (#171–#193 on origin/main). **CI DIAGNOSIS (#193):** owner reports NO Actions run visible for #181. Root cause is NOT the workflow — `.github/workflows/verify.yml` is correct, tracked, pushed since #181 (`329d517`), and triggers on every `push:`/`pull_request:`. With no run appearing at all, GitHub **Actions is almost certainly DISABLED for the repo**. **OWNER FIX (owner-only, gh unauth here):** repo → Settings → Actions → General → "Allow all actions and reusable workflows" → Save; then push any commit (the two #193 pushes will trigger it) or use Re-run. Until enabled, "CI is the arbiter" and single-machine-loss safety net do NOT hold. |
...
| 0.3 | Deploy: Vercel + Neon, set `DATABASE_URL`, `AUTH_SECRET`, `DATA_ENCRYPTION_KEY`, `SIGNUP_ALLOWLIST`, `CRON_SECRET`, `RESEND_API_KEY`, `VAPID_*`, `SENTRY_DSN` per docs/DEPLOY.md. Verify crons fire. | Human (keys) + Haiku (checklist walkthrough) | low | 10k | [!] BLOCKED (owner-only): no `VERCEL_TOKEN`/`NEON_API_KEY`/CLI/`​.vercel` link in this env, `gh` unauth, prod secrets live in the Google-Drive crash-backup folder. Code/config side confirmed ready: `vercel.json` wires all 4 crons; DEPLOY.md complete. Needs owner dashboard/CLI action. |
