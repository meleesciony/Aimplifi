...
| 0.3 | Deploy: Vercel + Neon, set `DATABASE_URL`, `AUTH_SECRET`, `DATA_ENCRYPTION_KEY`, `SIGNUP_ALLOWLIST`, `CRON_SECRET`, `RESEND_API_KEY`, `VAPID_*`, `SENTRY_DSN` per docs/DEPLOY.md. Verify crons fire. | Human (keys) + Opus (guided walkthrough) | low | 10k | **[~] PARTIAL 2026-07-10 (#198 + Resend).** Prod live; `SIGNUP_ALLOWLIST`/`CRON_SECRET`/`VAPID_*`/`RESEND_API_KEY` set + redeployed. Still optional: `SENTRY_DSN`. Cron *fire* UNVERIFIED. Confirm Resend domain for `reminders@aimplifi.app`. **Do not seed.** |
