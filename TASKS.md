...
| 0.3 | Deploy: Vercel + Neon, set `DATABASE_URL`, `AUTH_SECRET`, `DATA_ENCRYPTION_KEY`, `SIGNUP_ALLOWLIST`, `CRON_SECRET`, `RESEND_API_KEY`, `VAPID_*`, `SENTRY_DSN` per docs/DEPLOY.md. Verify crons fire. | Human (keys) + Opus (guided walkthrough) | low | 10k | **[~] NEXT PRIORITY (owner decision #196, 2026-07-09): do this first.** Owner-only actions (Vercel/Neon dashboard + keys — prod secrets in the Google-Drive crash-backup folder), agent drives the DEPLOY.md checklist step-by-step + verifies config/cron readiness. Code/config side confirmed ready: `vercel.json` wires all 4 crons; DEPLOY.md complete. |
